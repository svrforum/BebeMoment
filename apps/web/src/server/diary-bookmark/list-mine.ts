...
export type BookmarkedDiaryEntry = JournalBookmark & {
  entry:
    | (JournalEntry & {
...
export async function listMyDiaryBookmarks(
  familyId: string,
  userId: string,
  ...
  media: MediaClient,
): Promise<{ items: BookmarkedDiaryEntry[]; nextCursor: string | null }> {
  const limit = params.limit ?? 30
  const cur = params.cursor ? decodeCursor(params.cursor) : null
  const cursorTs = cur ? new Date(cur.ts) : null

  const items = await prismaPublic.journalBookmark.findMany({
    where: {
      familyId,
      userId,
      ...(cursorTs
  ...
  const entries = entryIds.length
    ? await prismaPublic.journalEntry.findMany({
        where: {
          id: { in: entryIds },
          familyId,
          deletedAt: null,
  ...
  const joined: BookmarkedDiaryEntry[] = page.map((b) => {
