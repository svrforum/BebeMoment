...
beforeEach(async () => { await db.prisma.oidcProvider.deleteMany() })
...
describe('oidc providers', () => {
  it('creates with encrypted client secret', async () => {
    const p = await createProvider(
      { name: 'Authentik', issuer: 'https://auth.example.com', clientId: 'bebe', clientSecret: 'topsecret', scopes: ['openid', 'email', 'profile'] },
      SECRET,
      db.prisma,
    )
